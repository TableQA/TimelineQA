...
def rearrange_table(table):
    ret_dict = {}
    def _type_converter(func, *args):
        try:
            func(*args)
            return 'real'
        except:
            return 'text'
    ret_dict['header'] = table['columns']
    ret_dict['rows'] = []
    for row in table['data']:
        ret_dict['rows'].append([str(cell) for cell in row])
    if len(table['data']):
        ret_dict['types'] = [_type_converter(float, a) for a  in table['data'][0]]
    else:
        ret_dict['types'] = [_type_converter(float, a) for a  in table['columns']]
...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            print()
            print("python multihopQA_Oracle.py -h -q <queryfile> -d <directories>")
        ...
        for d in directories:
            ...
            print(f'Writing {os.path.join(d,"oracle_table.jsonl")}...')
            table_file = open(os.path.join(d,'oracle_table.jsonl'), encoding='UTF-8', mode='w')
            with open(queryfile, "r") as f:
                ...
                for row in obj:
                    ...
                    if df.size > 0:
                        ...
                        if queryid in ['q2', 'q3', 'q4', 'q6', 'q10', 'q16', 'q18', 'q23', 'q24', 'q25']:
                            ...
                            if len(variables) > 0:
                                ...
                                oracle_query=oracle_query.format(**format_dict)
                        ...
                        queries_data['oracle_query'].append(oracle_query)
                        # print(query)
                        # print(oracle_query)
                        result = mysql(query)
                        gold_table = mysql(oracle_query)
                    else:
                        result = pd.DataFrame()
                    ...
                    gold_table = gold_table.to_json(orient="split")
                    gold_table = json.loads(gold_table)
